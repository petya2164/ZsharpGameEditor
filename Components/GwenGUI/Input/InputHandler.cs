...
namespace Gwen.Input
{
    ...
    public static class InputHandler
    {
        ...
        /// </summary>
        public static GUIControl KeyboardFocus;

        /// <summary>
        /// Control that currently has mouse focus.
        /// </summary>
        public static GUIControl MouseFocus;
        ...
        public static bool DoSpecialKeys(GUIControl canvas, char chr)
        {
        ...
        public static void OnCanvasThink(GUIControl control)
        {
            ...
            for (int i = 0; i < (int)Key.Count; i++)
            {
                ...
                if (m_KeyData.KeyState[i] && time > m_KeyData.NextRepeat[i])
                {
                    ...
                    {
                        KeyboardFocus.InputKeyPressed((Key)i, true);
                    }
                }
            }
        }
        ...
        /// <param name="dx"></param>
        /// <param name="dy"></param>
        public static void OnMouseMoved(GUIControl canvas, int x, int y, int dx, int dy)
        {
        ...
        public static bool OnMouseClicked(GUIControl canvas, int mouseButton, bool down)
        {
                ...
                case 0:
                    {
                        ...
                        if (isDoubleClick)
                            return HoveredControl.InputMouseDoubleClickedLeft(MousePosition.X, MousePosition.Y);
                        else
                            return HoveredControl.InputMouseClickedLeft(MousePosition.X, MousePosition.Y, down);                        
                    }

                case 1:
                    {
                        if (isDoubleClick)
                            return HoveredControl.InputMouseDoubleClickedRight(MousePosition.X, MousePosition.Y);
                        else
                            return HoveredControl.InputMouseClickedRight(MousePosition.X, MousePosition.Y, down);                        
                    }
            }
        ...
        /// <returns>True if handled.</returns>
        public static bool OnKeyEvent(GUIControl canvas, Key key, bool down)
        {
            if (null == KeyboardFocus) return false;
            if (KeyboardFocus.GetCanvas() != canvas) return false;
            if (!KeyboardFocus.IsVisible) return false;
            ...
            if (down)
            {
                if (!m_KeyData.KeyState[iKey])
                {
                    ...
                    return KeyboardFocus.InputKeyPressed(key, true);
                }
            }
            else
            {
        ...
        private static void UpdateHoveredControl(GUIControl inCanvas)
        {
            GUIControl hovered = inCanvas.GetControlAt(MousePosition.X, MousePosition.Y);
        ...
        private static void FindKeyboardFocus(GUIControl control)
        {
            ...
            FindKeyboardFocus(control.ParentControl);
            return;
        }
    }
}
