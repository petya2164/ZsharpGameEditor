...
namespace Gwen.Skin.Texturing
{
    ...
    public struct Bordered
    {
        ...
        public Bordered(Texture texture, float x, float y, float w, float h, Margin inMargin) // 1
            : this()
        {
            ...
            Init(texture, x, y, w, h, inMargin, 1.0f);
        }

        public Bordered(Texture texture, float x, float y, float w, float h, Margin inMargin, float drawMarginScale) // 1
            : this()
        {
            m_Rects = new SubRect[9];
            for (int i = 0; i < m_Rects.Length; i++)
        ...
        private void Init(Texture texture, float x, float y, float w, float h, Margin inMargin, float drawMarginScale)
        {
            m_Texture = texture;
