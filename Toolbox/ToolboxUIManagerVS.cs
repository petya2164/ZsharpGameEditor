...
namespace ToolboxLibrary
{
	...
	public class ToolboxUIManagerVS
	{
		...
		private void button_Click(object sender, EventArgs e)
		{
			...
			{
                // If the next tab is already at the bottom then move everything back up
                if (index + 1 < Toolbox.TabPageArray.Length && Toolbox.TabPageArray[index+1].Dock == DockStyle.Bottom)
                {
                    for (int i = index + 1; i < Toolbox.TabPageArray.Length; i++)
                        Toolbox.TabPageArray[i].Dock = DockStyle.Top;
                    index = Toolbox.TabPageArray.Length - 1;
                }
                else
                {
                    for (int i = index + 1; i < Toolbox.TabPageArray.Length; i++)
